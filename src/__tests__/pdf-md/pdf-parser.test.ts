...
import { ElementType, HeadingElement, ParagraphElement, PDFParsingError } from '../../pdf-md/types.js';
...
describe('PDF Parser', () => {
  ...
  test('parsePdf throws PDFParsingError when PDF parsing fails', async () => {
    ...
    await expect(parsePdf(mockBuffer)).rejects.toThrow(PDFParsingError);
  });
});